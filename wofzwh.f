...
C  1994 BY J.A.C. WEIDEMAN.THE USER CAN CHOOSE BETWEEN N=4, N=8 AND N=16.
      ...
      EXTERNAL RPOLYEV
      DOUBLE PRECISION CFFT(16), L, N, XABS, YABS,
...
C      WRITE (*,*) Z1
      ...
      SR = REAL(Z2)
      SI = AIMAG(Z2)
C     PROTECTION AGAINST OVERFLOW 
C      XABS = DABS(XI)
C      YABS = DABS(YI)
C      XABSQ = XABS**2
C      XQUAD = XABSQ - YABS**2
C      YQUAD = 2*XABS*YABS     
C      IF ((XABS.GT.RMAXREAL).OR.(YABS.GT.RMAXREAL)) GOTO 100
C      IF ((YQUAD.GT.RMAXGONI).OR.
C     *        (XQUAD.GT.RMAXEXP)) GOTO 100 
      CALL RPOLYEV(16,SR,SI,CFFT,QR,QI,PVR,PVI)
      P = CMPLX(PVR,PVI)
      Z2 = A+B*P+C
      IF (YI.LT.0) THEN
         Z2=-Z2
